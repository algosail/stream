...
export function multicast (strm) {
  ...
  const onEvent = (value) => notify (sinks, (s) => s.event (value))
  const onEnd = (reason) => notify (sinks, (s) => s.end (reason))
...
export function hold (strm) {
  ...
  return stream ((snk) => {
    ...
    if (outerDsp === null) {
      const onEvent = (value) => {
        last = M.just (value)
        notify (sinks, (s) => s.event (value))
      }

      const onEnd = (reason) => {
        sourceDone = true
        outerDsp = null
        notify (sinks, (s) => s.end (reason))
      }
...
export function bus () {
  ...
  const dispatch = (value) => notify (sinks, (s) => s.event (value))
